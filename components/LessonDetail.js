...
export const openLessonDetail = (state, lessonId, save, rerender) => {
  ...
  document.getElementById("detailLevel").textContent = getLevelLabel(
    lesson.level,
  );
  document.getElementById("detailTopics").textContent = (lesson.topics || [])
    .map(getTopicLabel)
    .join(" • ");
...
const renderQuiz = (state, lessonId, save, rerender) => {
  ...
  if (previous) {
    badge.textContent = previous.correct
      ? "Quiz concluído (acertou)"
      : "Quiz concluído (errou)";
    badge.className =
      "badge " + (previous.correct ? "badge-success" : "badge-warning");
  }
  ...
  ["A", "B", "C", "D"].forEach((letter) => {
    ...
    if (previous && previous.selected === letter) {
      b.classList.add(
        previous.correct ? "quiz-option-correct" : "quiz-option-incorrect",
      );
    }
