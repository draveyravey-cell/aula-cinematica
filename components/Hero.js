export const renderHero = (state) => {
  document.getElementById("heroLessonsCount").textContent =
    state.lessons.length.toString();
  document.getElementById("heroQuizCount").textContent =
    state.quizzes.length.toString();
};
export const setupHeroActions = (state, getFiltered, openLesson, rerender) => {
  document.getElementById("startBasicsBtn").addEventListener("click", () => {
    ...
    document
      ...
      .forEach((btn) =>
        btn.classList.toggle("pill-active", btn.dataset.level === "basico"),
      );
    rerender();
  ...
  document.getElementById("continueBtn").addEventListener("click", () => {
    ...
    if (completedIds.length) {
      ...
      const lastIndex = state.lessons.findIndex(
        (l) => l.id === lastCompletedId,
      );
      const index =
        lastIndex >= 0 && lastIndex < state.lessons.length - 1
          ? lastIndex + 1
          : 0;
